/**
* name: 为html生成目录 html.js: 顶部0个，左下角一个
* version: 0.1
...
function addContents(){
	...
	var	oContent=getElementsByClassName('content')[0],
		aH=oContent.querySelectorAll("h1,h2,h3,h4,h5,h6");
		oUl=createElement('ol');

	//添加内容
	for(var i=0;i<aH.length;i++){
		var j=i+1;//从1开始，而不是0.
		//var text=aH[i].innerHTML;
		...
		if(trim(text)!=''){
			...
			var oH = aH[i]
			oH.appendChild( createElement('a',{
				'class':'smallA',
				'name':j,
				'my-data':'anchor',
				'style':"float: right; color:#ddd; font-size:13.6px; font-family:monospace; font-weight: normal; /*visibility: hidden;*/"
			}, '[Section '+j+']' ));
	...
	//2. add contents
	//oContent.insertBefore( oUl, aH[0]); //加入文档流

	//3.加入左下角菜单中
	$("f_content").getElementsByTagName("div")[0].appendChild( oUl.cloneNode(true) );

	//3. add "正文"
	//oContent.insertBefore( createElement('h2',{},'正文' ), aH[0]); //加入文档流
}
